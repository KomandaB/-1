...
if ( ID == 2)
{
    ui->Textbatlle->setText(ui->Textbatlle->toPlainText()+ "\n"+  "Вы шли дальше и дальше, этот переход был особенно длинными, время от времени раздавались странные звуки из вентиляции. Вы начинаете разбирать звуки – это нечто похожее на рычание. Ваше оружие наготове. Вы ускоряете шаг и идете все быстрее и быстрее… Вы видите впереди дверь. Она открывается, Человек в форме кричит – «Беги, дебил, оно тебя сожрет». Но тут вентиляция сверху обрушивается, и пред вами предстает какое-то чудовище, похожее на огромную собаку-мутанта. Чудовище хрипит, с трудом вы разбираете этот хрип: «Едаааааа….». Не медля ни секунды, вы выхватываете пушку и бросаетесь прикончить монстра.");
}
if ( ID == 3)
{
    ui->Textbatlle->setText(ui->Textbatlle->toPlainText()+ "\n"+  "Вы решаете совершить благородный поступок и помочь жителям Пушкинской с освоением новой станции. Идя по тоннелю вы не наблюдаете ни какой активности. Полная тишина. Вам кажется это очень странным, ведь да же крысы, вечные жители метрополитена отсутствуют. Идя все дальше, вы упираетесь в металлическую ограду. Внезапно стена позади вас зашевелилась, и пред вами предстало каменное чудовище из мифов и легенд. Кого только не встретишь в метро!! Ну что, герой , в бой!?");
}
if ( ID == 4)
{
    ui->Textbatlle->setText(ui->Textbatlle->toPlainText()+"\n"+  "По дороге на Обводный канал, вы идете максимально Тихо и осторожно. Вдруг ваш фонарь начинает мигать и тускнеть. Впереди вы видите тень. Знакомый фантом, пронеслось у вас в голове . Вы незаметно подкрадываетесь и наносите удар .");
}
if ( ID == 5)
{
  ui->Textbatlle->setText(ui->Textbatlle->toPlainText()+ "\n"+  "Финальный бос. Могу ли я его одлеть?");
}
...
void FaitWindow::monster_is_dead()
{
    ...
    {
        ui->Textbatlle->setText(ui->Textbatlle->toPlainText()+ "\n"+  "Вы побеждаете это чудовище и бежите к бойцу, двери закрываются, и вы слышите нечеловеческий вой, а затем кто-то силой начинает долбить и царапать дверь. Но, она вполне крепкая, и выдерживает натиск с легкостью. Вы идете с незнакомцем на станцию. -Пойдем, сказала девушка.");
    }
    if ( currentMonsterId == 3)
    {
        ui->Textbatlle->setText(ui->Textbatlle->toPlainText()+ "\n"+  "Уворот, рывок, и вот чудовище получает сокрушительный удар и падает на ограду, подмяв ее под себя, освободив вам путь дальше. А дальше вы находите кучу гильз, видимо много кто хотел покорить этот путь. Неплохая наград для героя?! Идем дальше?");
    }
    if ( currentMonsterId == 4)
    {
        ui->Textbatlle->setText(ui->Textbatlle->toPlainText()+"\n"+  "Как бывалый воин, победив врага вы идете дальше. Чем дальше вы продвигаетесь, тем меньше в вас остается человечности.");
    }
